...
class CLITests(unittest.TestCase):
    ...
    def test_read_only_workflow(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            code, output, errors = self.run_cli("doctor", "--steam-root", str(steam), "--json")
            ...
            code, output, _ = self.run_cli("plan-import", str(steam / "config" / "stplug-in" / "10.lua"), "--steam-root", str(steam))
            self.assertEqual(code, 0)
    ...
    def test_catalog_and_backup_workflow(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            code, _, errors = self.run_cli("scan", "--catalog", str(catalog), "--steam-root", str(steam))
            self.assertEqual((code, errors), (0, ""))
            code, output, _ = self.run_cli("catalog", "--catalog", str(catalog), "stats", "--json")
            self.assertEqual(code, 0)
            self.assertIn('"games": 2', output)
            backup = root / "backup"
            code, output, _ = self.run_cli("backup", "--output", str(backup), "--steam-root", str(steam))
            self.assertEqual(code, 0)
    ...
    def test_invalid_explicit_root_fails(self):
        with tempfile.TemporaryDirectory() as directory:
            code, _, errors = self.run_cli("games", "--steam-root", str(Path(directory) / "missing"))
    ...
    def test_integrated_profiles_snapshots_managed_lua_and_config_io(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            code, output, errors = self.run_cli("profiles", "--steam-root", str(steam), "--json")
            self.assertEqual((code, errors), (0, ""))
            self.assertIn('"account_id": "7"', output)

            before = root / "before.json"
            after = root / "after.json"
            code, _, errors = self.run_cli("snapshot", "create", str(before), "--steam-root", str(steam))
            self.assertEqual((code, errors), (0, ""))
            code, _, _ = self.run_cli("snapshot", "create", str(after), "--steam-root", str(steam))
            self.assertEqual(code, 0)
            code, output, _ = self.run_cli("snapshot", "diff", str(before), str(after), "--json")
            self.assertEqual(code, 0)
            self.assertIn('"changed": 0', output)

            code, output, _ = self.run_cli("managed-lua", "list", "--steam-root", str(steam), "--json")
            self.assertEqual(code, 0)
            ...
            code, _, _ = self.run_cli("--config", str(config), "config", "export", str(exported))
            self.assertEqual(code, 0)
            code, output, _ = self.run_cli(
                "--config", str(root / "imported.json"), "config", "import", str(exported), "--json"
            )
            self.assertEqual(code, 0)
            self.assertIn('"applied": false', output)
