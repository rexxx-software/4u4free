...
from four_u_four_free.settings_io import (
    export_config,
    import_config,
    read_config_export,
)
...
class SettingsIOTests(unittest.TestCase):
    def test_export_preview_import_and_backup(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            source_store.save(
                AppConfig(
                    steam_root=str(root / "Steam"),
                    preferred_library=str(root / "Library"),
                )
            )
            ...
            self.assertEqual(
                target_store.load().preferred_library, str(root / "Library")
            )
