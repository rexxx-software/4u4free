    ...
    def test_backup_verify_dry_run_and_restore(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            backup = create_backup(
                steam, root / "backup", [steam, root / "Library Two"]
            )
            ...
            external_manifest = (
                root / "Library Two" / "steamapps" / "appmanifest_20.acf"
            )
            ...
            result = restore_backup(
                ...
                steam,
                apply=True,
                pre_restore_output=root / "pre-restore",
            )
            self.assertTrue(result["applied"])
            self.assertEqual(
                (steam / "config" / "config.vdf").read_text(encoding="utf-8"), original
            )
            self.assertEqual(
                external_manifest.read_text(encoding="utf-8"), external_original
            )
