...
from four_u_four_free.snapshots import (
    compare_snapshots,
...
class SnapshotTests(unittest.TestCase):
    def test_snapshot_round_trip_and_diff(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            manifest.write_text(
                manifest.read_text(encoding="utf-8").replace('"123"', '"124"'),
                encoding="utf-8",
            )
            (second / "steamapps" / "appmanifest_20.acf").unlink()
            (second / "steamapps" / "appmanifest_30.acf").write_text(
                '"AppState" { "appid" "30" "name" "Day of Defeat" "buildid" "1" }',
                encoding="utf-8",
            )
    ...
    def test_invalid_snapshot_is_rejected(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            path.write_text(
                json.dumps({"schema_version": 1, "games": [{"app_id": "oops"}]}),
                encoding="utf-8",
            )
