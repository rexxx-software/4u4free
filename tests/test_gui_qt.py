...
class QtGuiTests(unittest.TestCase):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            try:
                with (
                    ...
                    patch(
                        "four_u_four_free.gui_qt.stop_headless_idle"
                    ) as stop,
                ):
                    window._confirm_start_playtime_session()
                    ...
                    self.assertIn("Headless helper stopped", window.playtime_remaining.text())
            finally:
                window.close()
    ...
    def test_missing_provider_metadata_has_actionable_download_status(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            try:
                with (
                    patch.object(window, "_require_sff"),
    ...
    def test_achievement_launch_requires_confirmation(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            try:
                ...
                launch.assert_called_once_with("42")
                self.assertIn("Opened for Example Game", window.achievement_status.text())
            finally:
                window.close()
    ...
    def test_native_window_chrome_and_bundled_font(self):
        ...
        try:
            ...
            self.assertTrue(window.windowFlags() & Qt.WindowType.WindowMinMaxButtonsHint)
    ...
    def test_tools_navigation_and_settings_dirty_state(self):
        ...
        try:
            ...
            self.assertEqual(
                window.settings_change_status.text(), "Unsaved changes"
            )
