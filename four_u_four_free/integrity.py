...
def _load_manifest(backup_dir: Path) -> Dict[str, object]:
    ...
    try:
        ...
        raise FourUFourFreeError(
            f"Could not read backup manifest {path}: {exc}"
        ) from exc
...
def _safe_backup_relative(value: object) -> Path:
    ...
    if len(path.parts) < 4 or path.parts[0] != "roots":
        raise FourUFourFreeError(
            f"Backup file path is outside the archive layout: {value!r}"
        )
    return path
...
def _safe_target_relative(value: object) -> Path:
    ...
    allowed = (
        ...
        and pure.parts[1] == "config.vdf",
        len(pure.parts) == 3
        and pure.parts[:2] == ("config", "stplug-in")
        and pure.suffix.lower() == ".lua",
        len(pure.parts) == 2
        and pure.parts[0] == "steamapps"
        and pure.parts[1] == "libraryfolders.vdf",
        len(pure.parts) == 2
        and pure.parts[0] == "steamapps"
        and pure.name.startswith("appmanifest_")
        and pure.suffix == ".acf",
    )
    if not any(allowed):
        raise FourUFourFreeError(
            f"Backup target is outside the restore allowlist: {value!r}"
        )
    return path
...
def _entry_paths(entry: Dict[str, object], schema_version: int) -> tuple[Path, Path]:
    if schema_version >= 2:
        return _safe_backup_relative(entry.get("backup")), _safe_target_relative(
            entry.get("target")
        )
...
def verify_backup(backup_dir: Path) -> VerificationResult:
    ...
    return VerificationResult(
        root, not missing and not mismatched, checked, missing, mismatched
    )
...
def restore_backup(
    ...
    schema_version = int(manifest.get("schema_version", 1))
    allowed_roots = {
        str(path.resolve(strict=False)).casefold(): path.resolve(strict=False)
        for path in list_libraries(root)
    }
    ...
    for entry in manifest["files"]:
        ...
        target_root_value = (
            entry.get("target_root", str(root)) if schema_version >= 2 else str(root)
        )
        ...
        if allowed_root is None:
            raise FourUFourFreeError(
                f"Backup targets an unregistered Steam library: {target_root}"
            )
    ...
    targets = [
        str(target_root / target_relative)
        for _, _, target_root, target_relative in resolved_entries
    ]
    if not apply:
        return {
            "applied": False,
    ...
    pre_restore: Optional[BackupResult] = create_backup(
        root, pre_restore_output, allowed_roots.values()
    )
    restored: List[str] = []
    try:
