...
def launch_steam_unelevated(
    steam_exe: Path, cwd: Path | None = None
) -> tuple[bool, str]:
    """Launch Steam without inheriting our elevation.

    Steam's manifest is `asInvoker`. When 4u4free runs elevated and tries to
    ...
    try:
        ...
        return False, (
                f"ShellExecute failed (code {ret}). Close 4u4free "
