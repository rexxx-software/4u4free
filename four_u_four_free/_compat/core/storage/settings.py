...
from four_u_four_free._compat.core.secret_store import keyring_decrypt, keyring_encrypt
from four_u_four_free._compat.core.structs import SettingCustomTypes, Settings
from four_u_four_free._compat.core.utils import app_data_dir

logger = logging.getLogger(__name__)

SETTINGS_FILE = app_data_dir() / "settings.bin"
...
def _atomic_write_settings(data: dict) -> None:
    tmp_fd, tmp_path = tempfile.mkstemp(
        prefix="settings.", suffix=".tmp", dir=SETTINGS_FILE.parent
    )
    try:
...
def get_setting(key):
    ...
    if value and key.hidden:
        try:
            ...
            logger.warning(
                "Failed to decrypt hidden setting %s — encryption key may have changed",
                key.clean_name,
            )
            return None
    return value
...
def export_settings(export_path, include_sensitive=False):
    try:
        settings = load_all_settings()
        export_data = {"version": SETTINGS_VERSION, "settings": {}}
    ...
    try:
        ...
        for key, value in import_data["settings"].items():
            ...
            if setting is None:
                ...
                continue
            if setting.type is bool and not isinstance(value, bool):
                ...
                continue
            elif setting.type is dict and not isinstance(value, dict):
        ...
        if errors:
            error_msg = f"Imported {imported_count} settings with errors: " + "; ".join(
                errors
            )
            logger.warning(error_msg)
            return True, error_msg
        logger.info(
            f"Successfully imported {imported_count} settings from {import_path}"
        )
