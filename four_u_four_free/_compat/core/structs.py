...
from four_u_four_free._compat.core.utils import root_folder
...
class Settings(Enum):
    ...
    HUBCAP_DISABLED = SettingItem(
        "hubcap_disabled", "Hubcap Store Disabled", False, bool
    )
    ...
    DEPOTBOX_RATE_LIMIT = SettingItem(
        "depotbox_rate_limit", "DepotBox Rate Limit (requests per minute)", False, str
    )
    ...
    CUSTOM_BACKGROUND_IMAGE = SettingItem(
        "custom_background_image", "Custom UI Background Image", False, str
    )
    CUSTOM_ACCENT_COLOR = SettingItem(
        "custom_accent_color", "Custom UI Accent Color", False, str
    )
    ONLINE_FIX_NEW_SYSTEM_SHOWN = SettingItem(
        "online_fix_new_system_shown", "Online-Fix New System Shown", False, bool
    )
    PARALLEL_DOWNLOADS = SettingItem(
        "parallel_downloads", "Parallel Download Workers", False, str
    )
    DOWNLOAD_CONCURRENCY = SettingItem(
        "download_concurrency", "Max concurrent chunk downloads (8-64)", False, str
    )
    DEPOT_DOWNLOAD_TIMEOUT = SettingItem(
        "depot_download_timeout",
        "DDMod depot timeout in minutes (0 = no limit)",
        False,
        str,
    )
    BACKUP_RETENTION = SettingItem(
        "backup_retention", "Backup Retention Count", False, str
    )
    ENABLE_NOTIFICATIONS = SettingItem(
        "enable_notifications", "Enable Desktop Notifications", False, bool
    )
    USE_PARALLEL_DOWNLOADS = SettingItem(
        "use_parallel_downloads", "Use Parallel Downloads", False, bool
    )
    ACTIVE_UNLOCKER_PER_GAME = SettingItem(
        "active_unlocker_per_game", "Active DLC Unlocker Per Game", False, dict
    )
    DLC_UNLOCKER_CACHE_DIR = SettingItem(
        "dlc_unlocker_cache", "DLC Unlocker Cache Directory", False, str
    )
    # DLC Unlocker mode (CreamInstaller-compatible)
    USE_SMOKEAPI = SettingItem(
        "use_smokeapi", "Prefer SmokeAPI over CreamAPI (Steam)", False, bool
    )
    HIDE_STORE_IMAGES = SettingItem(
        "hide_store_images", "Hide Store Images", False, bool
    )
    USE_MANIFEST_PINS = SettingItem(
        "use_manifest_pins", "Use Pinned Manifest Versions from Lua", False, bool
    )
    MANIFEST_PINS_ASKED = SettingItem(
        "manifest_pins_asked",
        "Manifest Pin Prompt Shown (managed automatically)",
        False,
        bool,
    )

    MANIFESTHUB_API_KEY = SettingItem(
        "manifesthub_api_key",
        "ManifestHub API Key (manifesthub2.filegear-sg.me, 24h)",
        True,
        str,
    )
    MANIFESTHUB_KEY_EXPIRY = SettingItem(
        "manifesthub_key_expiry",
        "ManifestHub Key Expiry (UTC epoch, managed automatically)",
        False,
        str,
    )
    LANGUAGE = SettingItem(
        "language", "Language (Requires Restart)", False, list(SupportedLanguages)
    )
    MANIFEST_UPDATE_EXCLUDES = SettingItem(
        "manifest_update_excludes", "Manifest Update Excluded Games", False, str
    )
    SAVE_WATCHER_INTERVAL = SettingItem(
        "save_watcher_interval",
        "Background Save Watcher Interval (minutes, 0=off)",
        False,
        str,
    )
    LAST_BACKUP_PROVIDER_CONFIG = SettingItem(
        "last_backup_provider_config",
        "Last Cloud Save Provider Config (managed automatically)",
        False,
        str,
    )
    CLOUD_PROVIDER = SettingItem("cloud_provider", "Cloud Save Provider", False, str)
    CLOUD_RCLONE_EXE = SettingItem(
        "cloud_rclone_exe", "Cloud Save rclone Executable", False, str
    )
    CLOUD_RCLONE_REMOTE = SettingItem(
        "cloud_rclone_remote", "Cloud Save rclone Remote", False, str
    )
    ...
    CLOSE_TO_TRAY = SettingItem(
        "close_to_tray", "Close button hides to tray (off = quit)", False, bool
    )
    ...
    AUTO_UPDATE_CHECK = SettingItem(
        "auto_update_check", "Check for updates on startup", False, bool
    )
    LAST_SKIPPED_VERSION = SettingItem(
        "last_skipped_version",
        "Last Skipped Update Version (managed automatically)",
        False,
        str,
    )
    ...
    # the work 4u4free registered. Default ON; the off-switch disables
    # backup, watcher, and restore in lockstep.
    MANIFEST_PRESERVE = SettingItem(
        "manifest_preserve",
        "Preserve manifests when Steam uninstalls a game (recommended)",
        False,
        bool,
    )
    ...
    STORE_SHOW_SOFTWARE = SettingItem(
        "store_show_software", "Show software in Store", False, bool
    )
    STORE_BLOCK_NSFW = SettingItem(
        "store_block_nsfw", "Block NSFW content in Store", False, bool
    )
    LIVE_LOG_MAX_LINES = SettingItem(
        "live_log_max_lines", "Live Log Line Limit", False, str
    )
    # LumaCore version tracking. Both fields are managed by four_u_four_free._compat.lumacore_setup
    ...
    # DarkH2o pattern. When ON, 4u4free drops a tiny override .lua into
    ...
    # JSON object {appid: bool}. True means 4u4free has dropped
