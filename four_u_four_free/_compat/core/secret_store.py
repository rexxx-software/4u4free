...
def _file_key_path() -> Path:
    p = app_data_dir() / ".4u4free_encryption_key"
    ...
    if _KEYRING_WORKING and keyring is not None:
        try:
            keyring.set_password(
                SERVICE, KEYNAME, _encode_key_material(key).decode("ascii")
            )
            return key
        except Exception as e:
...
def generate_key_and_ciphertext(plaintext: str):
    ...
    ciphertext = _encode_key_material(
        SecretBox(raw_key).encrypt(plaintext.encode("utf-8"))
    )
