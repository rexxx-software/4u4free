...
from four_u_four_free._compat.game.online_fix import (
    _extract_archive_with_backup,
    _detect_archiver,
)
...
def search_crack_games(query: str, all_games: list[dict]) -> list[dict]:
    ...
    contains = [
        g for g in all_games if q in g.get("name", "").lower() and g not in exact
    ]
    return exact + contains


def _badge_summary(game: dict) -> str:
    badges = set(b for f in game.get("fixes", []) for b in f.get("badges", []))
    return ", ".join(sorted(badges)) if badges else ""


def _extract_to_game_folder(
    archive_path: Path, game_folder: Path, game_name: str
) -> bool:
    ...
    try:
        if ext == ".zip":
            ...
            if not archiver_type:
                print(
                    Fore.RED
                    + "No archiver found. Install 7-Zip or WinRAR."
                    + Style.RESET_ALL
                )
                return False
...
def apply_crack_fix(game_name: str, game_folder) -> bool:
    ...
    if not matches:
        print(
            Fore.YELLOW
            + f"No match for '{game_name}'. Select manually."
            + Style.RESET_ALL
        )
        options = [
            (_fmt(g), g) for g in sorted(all_games, key=lambda x: x.get("name", ""))
        ]
    else:
    ...
    if len(fixes) == 1:
        ...
        chosen_fix = prompt_select(
            "Select download source:", fix_options, cancellable=True
        )
    ...
    if not file_id:
        print(
            Fore.RED + f"Could not parse pixeldrain ID from: {href}" + Style.RESET_ALL
        )
        return False

    temp_dir = Path(tempfile.mkdtemp(prefix="4u4free_crack_fix_"))
    try:
