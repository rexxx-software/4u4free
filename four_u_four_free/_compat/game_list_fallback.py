...
def _get_ssl_ctx():
    ...
    try:
        ...
        logger.warning(
            "No CA certificates available, HTTPS verification disabled for game list fetches"
        )
...
_GAMES_JSON_URL = "https://raw.githubusercontent.com/SteamTools-Team/GameList/refs/heads/main/games.json"
_NAME_JSON_URLS = {
    "games_appid.json": [
...
def _get_cache_dir() -> Path:
    ...
    path = app_data_dir() / "store_metadata"
    path.mkdir(parents=True, exist_ok=True)
    return path


def _bundled_store_metadata_dir() -> Path | None:
    """Return the read-only metadata directory bundled with 4u4free."""
    try:
        bundled = Path(__file__).resolve().parents[1] / "data" / "store_metadata"
        if bundled.is_dir():
            return bundled
    except Exception:
        pass
...
def _normalize_name_data(data) -> dict:
    ...
    if isinstance(data, list):
        ...
        return out
    logger.warning(
        "appid name list: expected dict or list, got %s", type(data).__name__
    )
    return {}
    ...
    with _LOAD_LOCK:
        ...
        try:
            request = _req.Request(
                _GAMES_JSON_URL, headers={"User-Agent": "4u4free/0.5.5"}
            )
...
def _load_name_source(
    fname: str, urls: list[str], *, force: bool, cache_dir: Path
) -> tuple[dict, float]:
    ...
    for url in urls:
        try:
            request = _req.Request(url, headers={"User-Agent": "4u4free/0.5.5"})
...
def _load_name_cache(force=False):
    ...
    with _LOAD_LOCK:
        ...
        for fname, urls in _NAME_JSON_URLS.items():
            normalized, mtime = _load_name_source(
                fname, urls, force=force, cache_dir=cache_dir
            )
...
def ensure_loaded(force=False):
    ...
    if force or not _loaded:
        ...
        if now - _FALLBACK_LOG_LAST > 60:
            logger.debug(
                "Fallback data: games=%d entries, name_cache=%d entries, dlc_cache=%d entries",
