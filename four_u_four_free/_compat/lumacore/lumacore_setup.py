...
def _extract_dlls_via_subprocess(
    ...
    with tempfile.TemporaryDirectory(prefix="4u4free_lc_ext_") as tmp:
        tmp_path = Path(tmp)
        if tool_type == "winrar":
...
_LUMACORE_RELEASE_API = (
    f"https://api.github.com/repos/{_LUMACORE_GITHUB_REPO}/releases/latest"
)

_LC_DLLS = ("dwmapi.dll", "xinput1_4.dll", "LumaCore.dll", "LumaCorePayload.dll")

_GL_MARKER = ".4u4free_gl_cleaned"
...
def _migrate_legacy_marker(
    steam_path: Path, callback: Optional[Callable[[str], None]]
) -> bool:
...
def _prewarm_lumacore_patterns(
    ...
    for label, binary_path, subdir in jobs:
        if not binary_path.is_file():
            _progress(
                f"Pattern cache skipped for {label}: Steam DLL not found.", callback
            )
            continue
        try:
...
def prewarm_pattern_cache_if_missing(steam_path) -> None:
    ...
    try:
        if not steam_path:
            return
        if (
            not (steam_path / "LumaCore.dll").is_file()
            and not (steam_path / "dwmapi.dll").is_file()
        ):
            return
    ...
    try:
        ...
        for priority_name in primary_names:
            for asset in assets:
                if asset.get("name", "").lower() == priority_name:
                    logger.debug(
                        "LumaCore release asset: %s (tag=%s)", asset["name"], tag_name
                    )
        ...
        for asset in assets:
            ...
            if name_lower.endswith(".zip") and name_lower.startswith(variant_lower):
                logger.debug(
                    "LumaCore release asset (loose match): %s (tag=%s)",
                    asset["name"],
                    tag_name,
                )
                return asset["browser_download_url"], asset["name"]
        ...
        for asset in assets:
            ...
            if name.lower().endswith(".zip") and "source code" not in name.lower():
                logger.debug(
                    "LumaCore release asset (fallback): %s (tag=%s)", name, tag_name
                )
...
def _dll_name_match(names: list[str], dll: str) -> Optional[str]:
    ...
    return next(
        (
            n
            for n in names
            if n.lower() == dll_lower or n.lower().endswith(f"/{dll_lower}")
        ),
        None,
    )
...
def _extract_zip(
    archive: Path, steam_path: Path, callback: Optional[Callable[[str], None]]
) -> bool:
    try:
        with zipfile.ZipFile(archive, "r") as zf:
            ...
            for dll in _LC_DLLS:
                ...
                try:
                    ...
                    _progress(
                        f"Permission denied writing {dll} — Steam may be running. Close Steam completely and try again.",
                        callback,
                    )
                    return False
                _progress(f"Installed {dll}", callback)
...
def install_lumacore(
    ...
    *variant* picks the asset flavour. "release" (default) pulls the
    ...
    with tempfile.TemporaryDirectory(prefix="4u4free_lc_") as tmp:
...
def _fetch_latest_release_tag(timeout: float = 10.0) -> Optional[str]:
    ...
    try:
        ...
        # display fallback.
        return (
            _normalise_lc_version(payload.get("tag_name") or payload.get("name") or "")
            or None
        )
    except Exception as exc:
...
def get_installed_lumacore_version(steam_path: Path) -> str:
    ...
    4u4free last installed, falling back to the version LumaCore itself
...
def remember_installed_lumacore_version(version: str) -> None:
    """Persist the version label 4u4free just installed."""
...
def check_for_lumacore_update(steam_path: Path, force: bool = False) -> dict:
    ...
    now = time.time()
    use_cache = (
        not force and cached_latest and (now - last_check) < _LC_VERSION_CHECK_INTERVAL
    )
...
# ─────────────────────────────────────────────────────────────────────────
#
    # Removes every file 4u4free installs as part of LumaCore. Steam MUST be
...
def _force_close_steam(
    ...
    if sys.platform != "win32":
        return
    try:
        from four_u_four_free._compat.core.processes import is_proc_running
    except Exception:
