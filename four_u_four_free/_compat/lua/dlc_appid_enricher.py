...
from four_u_four_free._compat.core.structs import DLCTypes
...
def _compute_depotless_set(provider: SteamInfoProvider, parent_appid: int) -> set:
    ...
    try:
        ...
        try:
            dlc_ids = [
                int(x.strip()) for x in listofdlc.split(",") if x.strip().isdigit()
            ]
        except (TypeError, ValueError):
            return set()
        if not dlc_ids:
        ...
        for dlc_id in dlc_ids:
            ...
            try:
                ...
                logger.debug(
                    "dlc_appid_enricher: ParsedDLC raised for %s: %s", dlc_id, e
                )
                continue
        return depotless
    except Exception as e:
        logger.debug(
            "dlc_appid_enricher: provider walk failed for %s: %s", parent_appid, e
        )
        return set()
