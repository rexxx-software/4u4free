...
from four_u_four_free._compat.lua.choices import (
    add_new_lua,
    download_lua,
    select_from_saved_luas,
)
from four_u_four_free._compat.ui.prompts import prompt_select
from four_u_four_free._compat.core.storage.named_ids import get_named_ids
...
_DEPOT_NO_KEY_REGEX = re.compile(r"^\s*addappid\s*\(\s*(\d+)\s*\)", flags=re.MULTILINE)
...
def parse_lua_contents(contents, path):
    ...
    return LuaParsedInfo(
        path, contents, app_id, depot_pairs, manifest_overrides, token_overrides
    )
    ...
    for line in lines:
        ...
        if manifest_line:
            ...
            if depot_id in pins:
                ...
                rewritten.append(
                    f'{indent}setManifestid({depot_id}, "{pins[depot_id]}")'
                )
...
def remove_depots_from_lua(path: Path, depot_ids: set[str]) -> int:
    ...
    lines = text.split("\n")
    _depot_re = re.compile(r"^\s*(?:set[Mm]anifestid|addappid)\s*\(\s*(\d+)\b")
    kept = []
    removed = 0
    for line in lines:
...
class LuaManager:
    ...
    def get_raw_lua(self, choice: LuaChoice, override: Optional[Path] = None):
        while True:
