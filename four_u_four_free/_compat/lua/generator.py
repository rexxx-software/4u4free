...
from four_u_four_free._compat.lua.provider import get_entry, is_valid_key, load_provider
from four_u_four_free._compat.lua.update_pins import _REDIST_DEPOTS
...
def _depot_from_any(
    item, manifests: dict | None = None, provider: dict | None = None
) -> LuaDepot | None:
    ...
    if isinstance(item, dict):
        ...
        return LuaDepot(
            ...
            parent_appid=str(
                item.get("parent_appid") or entry.get("parent_appid") or ""
            ),
            parent_name=str(item.get("parent_name") or entry.get("parent_name") or ""),
            manifest_id=str(
                item.get("manifest_id") or (manifests or {}).get(depot_id) or ""
            ),
...
def render_grouped_lua(
    ...
    manifests = {
        str(k): str(v)
        for k, v in (manifest_overrides or {}).items()
    ...
    for raw in depots:
        ...
        if depot.parent_appid and depot.parent_appid != app_id_str:
            ...
            depot.parent_appid = (
                "1007" if int(depot.depot_id) in (1004, 1005, 1006) else "228980"
            )
            shared_depots.append(depot)
        else:
    ...
    if main_depots:
        ...
        for depot in sorted(main_depots, key=lambda d: int(d.depot_id)):
            ...
            if depot.manifest_id:
                ...
                lines.append(
                    f'setManifestid({depot.depot_id}, "{depot.manifest_id}"{size_part})'
                )
    ...
    for raw in dlcs:
        ...
        if (
            not dlc.app_id
            or not dlc.app_id.isdigit()
            or dlc.app_id == app_id_str
            or dlc.app_id in seen_depots
        ):
            continue
