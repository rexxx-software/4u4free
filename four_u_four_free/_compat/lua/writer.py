...
from four_u_four_free._compat.network.http_utils import get_game_name

from four_u_four_free._compat.ui.prompts import prompt_confirm

from four_u_four_free._compat.core.storage.vdf import (
    VDFLoadAndDumper,
    vdf_dump,
    vdf_load,
)

from four_u_four_free._compat.core.structs import LuaParsedInfo
...
def _manifest_map_for_lua(
    lua: LuaParsedInfo, manifest_override: Optional[dict]
) -> dict[str, str]:
...
class ACFWriter:
        ...
        if do_write_acf:
            ...
            if manifest_map:
                ...
                print(
                    ...
                    + ", ".join(f"{d}:{m}" for d, m in list(manifest_map.items())[:3])
    ...
    def write_acf_direct(
        ...
        if sys.platform == "win32":
            logger.debug(
                "write_acf_direct: skipped on Windows (LumaCore handles ownership)"
            )
            return
        app_name = get_game_name(lua.app_id)
        app_id_str = str(lua.app_id)
        ...
        try:
            ...
            if patched:
                ...
                try:
                    ...
                    pass
                print(
                    "Patched .acf error state (cleared UpdateResult / validation flags)"
                )
            else:
    ...
    def patch_acf_depot_manifests(self, acf_file: Path, manifest_map: dict):
        ...
        try:
            ...
            logger.warning(
                "Could not patch ACF depot manifests for %s: %s", acf_file, e
            )
