...
from four_u_four_free._compat.lua.endpoints import (
    get_hubcap,
    get_oureverday,
    get_ryuu,
    get_depotbox,
)
from four_u_four_free._compat.ui.prompts import prompt_file, prompt_select, prompt_text
from four_u_four_free._compat.core.storage.settings import get_setting
from four_u_four_free._compat.core.strings import STEAM_WEB_API_KEY
from four_u_four_free._compat.core.structs import (
    LuaChoice,
    LuaChoiceReturnCode,
    LuaEndpoint,
    LuaResult,
...
from four_u_four_free._compat.core.utils import enter_path
...
def _game_list_file() -> Path:
    return Path(__file__).resolve().parents[1] / "data" / "all_games.txt"
...
def _fetch_steam_apps() -> list[dict]:
    ...
    for label, url in fallback_urls:
        try:
            ...
            if isinstance(data, dict):
                for app_id_str, name in data.items():
                    if app_id_str.isdigit():
                        fallback_apps.append(
                            {"appid": int(app_id_str), "name": str(name)}
                        )
            ...
            if fallback_apps:
                print(
                    Fore.GREEN
                    + f"Found {len(fallback_apps):,} games from {label}"
                    + Style.RESET_ALL
                )
    ...
    if apps:
        print(
            Fore.YELLOW
            + f"Steam API partial success — got {len(apps):,} games before failure."
            + Style.RESET_ALL
        )
        return apps
    ...
    print(
        Fore.RED
        + "No game list available. Enter the App ID directly."
        + Style.RESET_ALL
    )
    return []
...
def _app_search_lines(apps: Iterable[dict]) -> list[str]:
    return [
        f"{app.get('name', 'UNKNOWN GAME')} [ID={app.get('appid')}]" for app in apps
    ]
...
def search_game(os_type):
    ...
    if isinstance(games_source, Path):
        choices = games_source.read_text(encoding="utf-8").splitlines()
    else:
        choices = games_source
    ...
    return _selected_app_id(selection)
