...
from four_u_four_free._compat.dlc_unlockers.base import (
    UnlockerBase,
    UnlockerType,
    Platform,
)
from four_u_four_free._compat.dlc_unlockers.steam_dll_utils import (
...
class SmokeAPIUnlocker(UnlockerBase):
    ...
    def _find_all_installation_locations(self, game_dir):
        ...
        return [
            InstallLocation(path=p, dll_name=name, architecture=arch)
            for p, name, arch in raw
        ]
    ...
    def _install_to_location(
        ...
        try:
            ...
            backup_dll_path = (
                location.path
                / f"{location.dll_name.replace('.dll', '')}{self.BACKUP_SUFFIX}.dll"
            )
            try:
                relative_path = location.path.relative_to(game_dir)
                relative_path_str = (
                    str(relative_path) if str(relative_path) != "." else "(root)"
                )
            ...
            if creamapi_config.exists():
                logger.info(
                    f"Deleted old CreamAPI configuration: {creamapi_config.name}"
                )
                creamapi_config.unlink()
            if not backup_dll_path.exists():
                if original_dll_path.exists():
                    logger.info(
                        f"Creating backup at {relative_path_str}: {location.dll_name} -> {backup_dll_path.name}"
                    )
                    shutil.copy2(original_dll_path, backup_dll_path)
                else:
                    logger.error(
                        f"Original DLL not found at {relative_path_str}: {original_dll_path}"
                    )
                    return False
            else:
                logger.info(
                    f"Backup already exists at {relative_path_str}: {backup_dll_path.name}"
                )
            if not smokeapi_dll_path.exists():
                logger.error(f"SmokeAPI DLL not found: {smokeapi_dll_path}")
                return False
            logger.info(
                f"Installing SmokeAPI to {relative_path_str}: {smokeapi_dll_path.name} -> {location.dll_name}"
            )
            shutil.copy2(smokeapi_dll_path, original_dll_path)
            if config is not None:
                config_path = location.path / self.CONFIG_FILENAME
                logger.info(
                    f"Writing configuration to {relative_path_str}: {self.CONFIG_FILENAME}"
                )
                ...
                if config_path.exists():
                    logger.info(
                        f"Deleted unnecessary configuration: {self.CONFIG_FILENAME}"
                    )
                    config_path.unlink()
    ...
    def _uninstall_from_location(self, location, game_dir):
        try:
            ...
            if not backup_dll_path.exists():
                logger.warning(
                    f"No backup found at {relative_path_str}: {backup_dll_path.name}"
                )
                return False
            logger.info(
                f"Restoring backup at {relative_path_str}: {backup_dll_path.name} -> {location.dll_name}"
            )
            if original_dll_path.exists():
                original_dll_path.unlink()
            shutil.copy2(backup_dll_path, original_dll_path)
            logger.info(
                f"Deleting backup at {relative_path_str}: {backup_dll_path.name}"
            )
            backup_dll_path.unlink()
            if config_path.exists():
                logger.info(
                    f"Deleting config at {relative_path_str}: {self.CONFIG_FILENAME}"
                )
                config_path.unlink()
    ...
    def _find_steam_api_dll(self, game_dir, dll_name):
        ...
        for found_dll in game_dir.rglob(dll_name):
            logger.info(
                f"Found {dll_name} in subdirectory: {found_dll.relative_to(game_dir)}"
            )
            return found_dll
        return None

    def install(self, game_dir, dlc_ids, app_id, smokeapi_dir=None):
        ...
        if not valid:
            ...
            logger.error("Try running with administrator privileges")
            return False
        valid, error = check_disk_space(
            game_dir, required_bytes=10 * 1024 * 1024
        )  # 10MB
        if not valid:
                ...
                if smokeapi_dir:
                    ...
                    if creaminstaller_dll.exists():
                        ...
                        if not smokeapi_dll_path.exists():
                            ...
                            logger.error(
                                f"Looked for: {location.dll_name} and {smokeapi_dll_name}"
                            )
                    ...
                    if not smokeapi_dll_path.exists():
                        logger.error(f"SmokeAPI DLL not found: {smokeapi_dll_path}")
                        logger.error(
                            "Please download SmokeAPI first using the downloader"
                        )
                        failures.append(location)
                        continue
                if self._install_to_location(
                    location, smokeapi_dll_path, config, game_dir
                ):
                    successes.append(location)
                else:
                    failures.append(location)
            logger.info(
                f"Installation summary: {len(successes)} succeeded, {len(failures)} failed"
            )
            if failures:
        ...
        try:
            ...
            if not locations:
                ...
                for config_path in game_dir.rglob(self.CONFIG_FILENAME):
                    try:
                        logger.info(
                            f"Removing orphaned config: {config_path.relative_to(game_dir)}"
                        )
                        config_path.unlink()
                    except Exception as e:
            ...
            for smokeapi_dll in [self.SMOKEAPI_32_DLL, self.SMOKEAPI_64_DLL]:
                for dll_path in game_dir.rglob(smokeapi_dll):
                    try:
                        logger.info(
                            f"Removing {smokeapi_dll}: {dll_path.relative_to(game_dir)}"
                        )
                        dll_path.unlink()
                    except Exception as e:
                        logger.warning(f"Could not remove {smokeapi_dll}: {e}")
            logger.info(
                f"Uninstallation summary: {len(successes)} succeeded, {len(failures)} failed"
            )
            if failures:
    ...
    def detect_proxy_mode(self, game_dir):
        ...
        proxy_dlls = ["version.dll", "winmm.dll", "winhttp.dll", "dsound.dll"]
                        ...
                        for module in config_data["modules"]:
                            if isinstance(module, dict) and "path" in module:
                                ...
                                if "smokeapi" in module_path:
                                    ...
                                    logger.info(
                                        f"Found Koaloader config referencing SmokeAPI: {config_name}"
                                    )
                                    break
                except (json.JSONDecodeError, KeyError, TypeError) as e:
                    logger.warning(
                        f"Could not parse Koaloader config {config_name}: {e}"
                    )
                            ...
                            for module in config_data["modules"]:
                                if isinstance(module, dict) and "path" in module:
                                    ...
                                    if "smokeapi" in module_path:
                                        proxy_files.append(config_path)
                                        logger.info(
                                            f"Found Koaloader config in subdirectory: {config_path.relative_to(game_dir)}"
                                        )
                                        break
                    except (json.JSONDecodeError, KeyError, TypeError) as e:
                        logger.warning(
                            f"Could not parse Koaloader config {config_path.relative_to(game_dir)}: {e}"
                        )
                        proxy_files.append(config_path)
        for dll_name in proxy_dlls:
            for dll_path in game_dir.rglob(dll_name):
                if dll_path not in proxy_files:
                    proxy_files.append(dll_path)
                    logger.info(
                        f"Found proxy DLL in subdirectory: {dll_path.relative_to(game_dir)}"
                    )
        return proxy_files
