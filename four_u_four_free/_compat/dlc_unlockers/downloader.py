...
from four_u_four_free._compat.network.http_utils import (
...
class GitHubReleaseDownloader:
    ...
    async def download_latest(self, unlocker_type: UnlockerType):
        ...
        if local_resource:
            logger.info(
                f"Using local resource for {unlocker_type.value} from {local_resource}"
            )
        ...
        for attempt in range(max_retries):
            try:
                ...
                if release_info:
                    ...
                    logger.warning(
                        f"Empty response, retrying ({attempt + 1}/{max_retries})..."
                    )
                ...
                logger.error(
                    f"Unexpected error fetching release info for {unlocker_type.value}: {e}"
                )
                return None
        if not release_info:
        ...
        with download_to_tempfile(download_url) as temp_file:
            if temp_file is None:
                ...
                return None
            try:
                from four_u_four_free._compat.zip import safe_extract_zip
                ...
                if dll_dir:
                    ...
                    logger.warning(
                        f"DLLs not found in expected location, returning root: {cache_subdir}"
                    )
                    return cache_subdir
            except Exception as e:
    ...
    def _find_dll_directory(self, root_dir, unlocker_type):
        ...
        for depth in range(
            3
        ):  # root, 1 level deep, 2 levels deep  # Check root, 1 level deep, 2 levels deep
            if depth == 0:
    ...
    def _get_local_resource(self, unlocker_type):
        ...
        if resource_dir.exists() and resource_dir.is_dir():
            logger.debug(
                f"Found local resource for {unlocker_type.value} at {resource_dir}"
            )
            return resource_dir
        logger.debug(
            f"No local resource found for {unlocker_type.value} at {resource_dir}"
        )
        return None
    ...
    def get_dll(self, unlocker_type, architecture):
        ...
        dll_names = {
            ...
            UnlockerType.CREAMAPI: {"32": ["steam_api.dll"], "64": ["steam_api64.dll"]},
            UnlockerType.UPLAY_R1: {
        ...
        for dll_name in possible_names:
            ...
            if dll_path.exists():
                ...
                return dll_path
        logger.warning(
            f"Could not find {architecture}-bit DLL for {unlocker_type.value} in {dll_dir}"
        )
        return None
