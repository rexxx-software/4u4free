...
from four_u_four_free._compat.core.storage.settings import load_all_settings
...
class UnlockerManager:
    ...
    def detect_platform(self, game_dir):
        if (game_dir / "steam_api.dll").exists() or (
        ...
        if (game_dir / "upc_r2_loader.dll").exists():
            ...
            return Platform.UBISOFT
        logger.warning(
            f"No platform-specific DLLs found in {game_dir}, defaulting to Steam"
        )
        return Platform.STEAM
    ...
    def get_compatible_unlockers(self, platform):
        ...
        logger.info(
            f"Found {len(compatible)} compatible unlockers for {platform.value}"
        )
        return compatible
    ...
    def get_active_unlocker(self, app_id):
        ...
        if unlocker_value:
            try:
                ...
                logger.warning(
                    f"Invalid unlocker type '{unlocker_value}' for app {app_id}"
                )
                return None
        return None
    ...
    def set_active_unlocker(self, app_id, unlocker_type):
        ...
        import msgpack
        from four_u_four_free._compat.core.storage.settings import SETTINGS_FILE
