...
class CreamAPIUnlocker(UnlockerBase):
    ...
    def _find_steam_api_dll(self, game_dir, dll_name):
        exclude_backup = (
            "_o" not in dll_name
        )  # When searching for backups, don't exclude
        return find_steam_api_dll(game_dir, dll_name, exclude_backup=exclude_backup)

    def _detect_architecture(self, game_dir):
        return detect_steam_architecture(
            game_dir, self.BACKUP_SUFFIX.replace(".dll", "")
        )
    ...
    def install(self, game_dir, dlc_ids, app_id):
        ...
        try:
            ...
            if not arch:
                return self._fail(f"Could not detect a Steam API DLL in {game_dir}.")
            ...
            creamapi_dll = None
            if self.downloader:
                creamapi_dll = self.downloader.get_dll(UnlockerType.CREAMAPI, arch)
            if not creamapi_dll or not creamapi_dll.exists():
                return self._fail(
                    f"Could not find the bundled CreamAPI DLL for {arch}-bit games."
                )
            if creamapi_dll.resolve(strict=False) == steam_api_path.resolve(
                strict=False
            ):
                return self._fail(
            ...
            if dll_replaced and backup_path and backup_path.exists() and steam_api_path:
                restore_tmp = (
                    steam_api_path.parent / f".{steam_api_path.name}.restore.tmp"
                )
                try:
    ...
    def _generate_ini_config(self, dlc_ids, app_id):
        ...
        lines.append(
            f"orgapi = {self.STEAM_API_32.replace('.dll', self.BACKUP_SUFFIX)}"
        )
        lines.append(
            f"orgapi64 = {self.STEAM_API_64.replace('.dll', self.BACKUP_SUFFIX)}"
        )
