...
class UplayR2Unlocker(UnlockerBase):
    ...
    def is_installed(self, game_dir):
        ...
        has_backup = (
            game_dir / f"{self.TARGET_DLL.replace('.dll', '')}{self.BACKUP_SUFFIX}.dll"
        ).exists()
        return has_config and has_backup

    def install(self, game_dir, dlc_ids, app_id, unlocker_dir=None):
        try:
            target_dll_path = game_dir / self.TARGET_DLL
            backup_dll_path = (
                game_dir
                / f"{self.TARGET_DLL.replace('.dll', '')}{self.BACKUP_SUFFIX}.dll"
            )
            ...
            if not backup_dll_path.exists():
                if target_dll_path.exists():
                    logger.info(
                        f"Backing up original {self.TARGET_DLL} to {backup_dll_path.name}"
                    )
            ...
            if not unlocker_dll_path.exists():
                ...
                logger.error(
                    "Please download Uplay R2 Unlocker first using the downloader"
                )
                return False
    ...
    def uninstall(self, game_dir):
        try:
            ...
            if backup_path.exists():
                logger.info(
                    f"Restoring backup: {backup_path.name} -> {self.TARGET_DLL}"
                )
                ...
                logger.warning(
                    f"No backup found for {self.TARGET_DLL}, leaving file in place"
                )
