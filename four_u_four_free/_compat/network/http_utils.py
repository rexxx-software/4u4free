...
from four_u_four_free._compat.ui.prompts import prompt_confirm, prompt_text
from four_u_four_free._compat.core.secret_store import b64_decrypt
...
async def _wait_for_enter():
    print(
        "If it takes too long, press Enter to cancel the request and input manually..."
    )
    hit = msvcrt.kbhit
    read = msvcrt.getch
    while True:
...
async def get_gmrc(manifest_id: Union[str, int], silent: bool = False):
    ...
    # against 4u4free. The two HTTPS fallbacks below cover the gmrc
    ...
    if sys.platform != "win32":
        ...
        request_task = asyncio.create_task(
            get_request(url, headers=headers, redact_url=True)
        )
    ...
    for fb_url in fallback_urls:
        fb_headers = {
            "Referer": get_base_domain(fb_url),
            **MANIFEST_REQUEST_CODE_HEADERS,
        }
        try:
    ...
    print(
            "  • ManifestHub API key → set in 4u4free settings → downloads manifests automatically"
    )
    ...
    code = prompt_text(
        "Paste the manifest request code (leave blank to skip): "
    ).strip()
    return code or None
