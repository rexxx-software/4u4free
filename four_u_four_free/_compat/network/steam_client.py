...
from four_u_four_free._compat.core.cache import get_cache
from four_u_four_free._compat.core.structs import DLCTypes, ProductInfo  # type: ignore
...
def _reopen_client_session(client):
    ...
    try:
        ...
        logger.debug(
            "Steam appinfo reconnect failed: %r (%s)", e, _client_state(client)
        )
    ...
    def _run():
        with _GEVENT_LOCK:
            ...
            for attempt, timeout in enumerate(_APP_INFO_TIMEOUTS, start=1):
                try:
                    return _ProductInfoResult(
                        _request_app_info(client, app_ids, timeout), True
                    )
                    ...
                    if attempt < _MAX_APP_INFO_RETRIES:
                        logger.debug(
                            "Request timed out after %ss. Trying again (%s/%s)...",
                            timeout,
                            attempt,
                            _MAX_APP_INFO_RETRIES,
                        )
                ...
                "4u4free will use cached/local manifests if available."
                    )
                    return _ProductInfoResult(_empty_product_info(), False)
            # All retries exhausted without an exception we recognised.
            if last_error is not None:
                logger.warning(
                    f"App info gave up after {_MAX_APP_INFO_RETRIES} attempts: {last_error}"
                )
            return _ProductInfoResult(_empty_product_info(), False)
...
class SteamInfoProvider:
    ...
    def _store_app_payloads(self, apps):
        for app_id, app_data in apps.items():
            ...
            self._persistent_cache.set(
                self._cache_key(app_id), app_data, ttl=7 * 24 * 3600
            )
    ...
    def _fill_from_http(self, app_ids):
        ...
        for app_id in app_ids:
            ...
            if payload:
                ...
                logger.debug("appinfo filled from SteamCMD HTTP mirror: %s", app_id)
    ...
    def get_app_info(self, app_ids, quick=False):
        ...
        if missing:
            ...
            if still_missing:
                result = _get_product_info_result(
                    self.client, still_missing, quick=quick
                )
