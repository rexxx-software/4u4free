    ...
    try:
        ...
        if isinstance(data, dict):
            raw = data.get("proxies") or (
                [data["proxy"]] if isinstance(data.get("proxy"), str) else []
            )
        ...
        print(
            Fore.YELLOW
            + f"  pixeldrain: proxy list fetch failed ({e}), falling back to cache"
            + Style.RESET_ALL
        )
        return []
...
def _stream_to_file(url: str, dest_dir: Path, fallback_name: str) -> Path | None:
    ...
    try:
        ...
        print(
            Fore.RED + f"  HTTP {e.response.status_code} from {url}" + Style.RESET_ALL
        )
    except Exception as e:
...
def download_pixeldrain(file_id: str, temp_dir: Path) -> Path | None:
    ...
    print(
        Fore.CYAN
        + f"Downloading from pixeldrain ({file_id}) via bypass..."
        + Style.RESET_ALL
    )
    ...
    if archive is None:
        ...
        print(
            Fore.YELLOW
            + "  All proxies failed. Trying direct pixeldrain..."
            + Style.RESET_ALL
        )
