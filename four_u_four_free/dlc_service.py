...
from sff.network.steam_store import (
...
def _dll_dir_for(downloader: GitHubReleaseDownloader, key: str):
    ...
    return (
        downloader.get_cached_dll(unlocker_type)
        or downloader._get_local_resource(unlocker_type)
    )
...
def install_unlocker(
    ...
    *,
    cache_dir: str | Path,
    downloader_factory: Callable[[Path], GitHubReleaseDownloader] = GitHubReleaseDownloader,
) -> bool:
    folder = require_game_directory(game_dir)
    app_id = require_app_id(app_id)
    ...
    if key == "smokeapi":
        return bool(
            SmokeAPIUnlocker().install(
                folder, dlc_ids, app_id, smokeapi_dir=dll_dir
            )
        )
    if key == "uplay-r1":
        return bool(
            UplayR1Unlocker().install(
                folder, dlc_ids, app_id, unlocker_dir=dll_dir
            )
        )
    return bool(
        UplayR2Unlocker().install(
            folder, dlc_ids, app_id, unlocker_dir=dll_dir
        )
    )
