...
try:
    ...
    from four_u_four_free._compat.dlc_unlockers.downloader import (
...
def cmd_doctor(args, store: ConfigStore) -> int:
    config = store.load()
    report = doctor(
        Path(args.steam_root) if args.steam_root else None, config.steam_root
    )
    if args.json:
        _json(report)
    else:
        _print_rows(
            (key.replace("_", " ").title(), value) for key, value in report.items()
        )
...
def cmd_games(args, store: ConfigStore) -> int:
    ...
    if args.json:
        ...
        rows.extend(
            (game.app_id, game.name, game.build_id or "-", game.library)
            for game in games
        )
        _print_rows(rows)
    return 0
    ...
    if args.json:
        ...
        if info.app_directives:
            ...
            for directive in payload["app_directives"]:
                print(
                    f"  {directive['app_or_depot_id']}  flag={directive['flag'] or '-'}  key={directive['key'] or '-'}"
                )
        if info.manifests:
            print("\nManifest pins:")
...
def cmd_restore(args, store: ConfigStore) -> int:
    ...
    if args.json:
        ...
        for target in result["targets"]:
            print(f"  {target}")
        print(
            "Run again with --apply to restore these files after an automatic pre-restore backup."
        )
    else:
...
def cmd_snapshot_diff(args, _store: ConfigStore) -> int:
    result = compare_snapshots(
        load_snapshot(Path(args.before)), load_snapshot(Path(args.after))
    )
    if args.json:
        _json(result)
    else:
...
def cmd_import_lua(args, _store: ConfigStore) -> int:
    catalog = Catalog(Path(args.catalog) if args.catalog else None)
    result = catalog.import_lua(
        Path(args.path), Path(args.archive) if args.archive else None
    )
    AuditLog().record(
        "lua.archived",
        {
            "sha256": result.sha256,
            "archived_path": str(result.archived_path),
...
def cmd_catalog_games(args, _store: ConfigStore) -> int:
    ...
    if args.json:
        ...
        table.extend(
            (row["app_id"], row["name"], row["build_id"] or "-", row["library"])
            for row in rows
        )
        _print_rows(table)
    return 0
...
def cmd_catalog_lua(args, _store: ConfigStore) -> int:
    ...
    if args.json:
        ...
        table.extend(
            (row["inferred_app_id"] or "-", row["sha256"], row["archived_path"])
            for row in rows
        )
        _print_rows(table)
    return 0
...
def cmd_report(args, store: ConfigStore) -> int:
    ...
    if args.output:
        ...
        AuditLog().record(
            "report.created", {"output": str(output), "steam_root": str(location.path)}
        )
...
def cmd_audit(args, _store: ConfigStore) -> int:
    ...
    if args.json:
        ...
        for record in records:
            print(
                f"{record.get('time', '-')}  {record.get('event', '-')}  {json.dumps(record.get('details', {}), ensure_ascii=False)}"
            )
    return 0
...
def cmd_config_set(args, store: ConfigStore) -> int:
    ...
    if args.steam_root is not None:
        config.steam_root = str(
            Path(args.steam_root).expanduser().resolve(strict=False)
        )
    if args.preferred_library is not None:
        config.preferred_library = str(
            Path(args.preferred_library).expanduser().resolve(strict=False)
        )
    store.save(config)
    AuditLog().record(
        "config.updated", {"config_file": str(store.path), "config": asdict(config)}
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--debug", action="store_true", help="Show full tracebacks on unexpected errors"
    )
    ...
    backup_parser = subparsers.add_parser(
        "backup", help="Create a checksummed Steam-state backup"
    )
    ...
    verify_parser = subparsers.add_parser(
        "verify-backup", help="Verify every file in a backup by SHA-256"
    )
    ...
    restore_parser = subparsers.add_parser(
        "restore", help="Verify and restore a backup; dry-run unless --apply is used"
    )
    restore_parser.add_argument("backup")
    restore_parser.add_argument(
        "--apply",
        action="store_true",
        help="Restore after creating an automatic pre-restore backup",
    )
    steam_options(restore_parser)
    restore_parser.set_defaults(handler=cmd_restore)

    snapshot_parser = subparsers.add_parser(
        "snapshot", help="Create or compare portable inventory snapshots"
    )
    snapshot_subparsers = snapshot_parser.add_subparsers(
        dest="snapshot_command", required=True
    )
    snapshot_create = snapshot_subparsers.add_parser(
        "create", help="Write the current Steam inventory to JSON"
    )
    ...
    snapshot_diff = snapshot_subparsers.add_parser(
        "diff", help="Compare two inventory snapshot files"
    )
    ...
    managed_parser = subparsers.add_parser(
        "managed-lua", help="Inspect or quarantine Steam plug-in Lua files"
    )
    managed_subparsers = managed_parser.add_subparsers(
        dest="managed_command", required=True
    )
    managed_list = managed_subparsers.add_parser(
        "list", help="List parsed plug-in Lua metadata with secrets redacted"
    )
    ...
    managed_quarantine.add_argument(
        "--apply",
        action="store_true",
        help="Back up Steam state and move matching files",
    )
    managed_quarantine.add_argument(
        "--quarantine", help="Override the quarantine parent directory"
    )
    managed_quarantine.add_argument(
        "--backup-output", help="Override the pre-change backup destination"
    )
    steam_options(managed_quarantine)
    managed_quarantine.set_defaults(handler=cmd_managed_quarantine)

    scan_parser = subparsers.add_parser(
        "scan", help="Index installed game manifests into the local catalog"
    )
    ...
    import_parser = subparsers.add_parser(
        "import-lua",
        help="Parse and archive Lua metadata locally without installing it",
    )
    import_parser.add_argument("path")
    import_parser.add_argument(
        "--archive", help="Override the immutable archive directory"
    )
    ...
    catalog_parser = subparsers.add_parser(
        "catalog", help="Query the local SQLite catalog"
    )
    ...
    report_parser = subparsers.add_parser(
        "report", help="Create a local Steam environment report"
    )
    report_parser.add_argument("--output")
    report_parser.add_argument(
        "--force", action="store_true", help="Replace an existing report file"
    )
    report_parser.add_argument("--steam-root", help="Explicit Steam installation root")
    report_parser.set_defaults(handler=cmd_report)

    audit_parser = subparsers.add_parser(
        "audit", help="Show recent local write-operation audit events"
    )
    ...
    config_parser = subparsers.add_parser(
        "config", help="Show or update local configuration"
    )
    ...
    config_export = config_subparsers.add_parser(
        "export", help="Export validated non-secret settings"
    )
    config_export.add_argument("output")
    config_export.add_argument(
        "--force", action="store_true", help="Replace an existing export"
    )
    ...
    config_import = config_subparsers.add_parser(
        "import", help="Validate or import a settings export"
    )
    config_import.add_argument("source")
    config_import.add_argument(
        "--apply",
        action="store_true",
        help="Back up current settings and import the export",
    )
    ...
    dlc_parser = subparsers.add_parser(
        "dlc-unlocker", help="Apply DLC unlockers (CreamAPI, SmokeAPI, Uplay R1/R2)"
    )
    dlc_parser.add_argument("game_path", help="Path to the game folder")
    dlc_parser.add_argument(
        "--unlocker",
        choices=["creamapi", "smokeapi", "uplay-r1", "uplay-r2"],
        help="Unlocker to apply",
    )
    dlc_parser.add_argument(
        "--app-id", type=int, help="Steam App ID (required for install)"
    )
    dlc_parser.add_argument(
        "--dlc-id", type=int, action="append", default=[], help="DLC App ID; repeatable"
    )
    dlc_parser.add_argument(
        "--list",
        action="store_true",
        help="List unlockers and whether each is installed",
    )
    dlc_parser.add_argument(
        "--validate",
        action="store_true",
        help="Show which platform DLLs exist in the game folder",
    )
    dlc_parser.add_argument(
        "--uninstall",
        action="store_true",
        help="Remove an installed unlocker and restore backups",
    )
    dlc_parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Preview the install plan without changing files",
    )
    dlc_parser.add_argument(
    ...
    lc_parser = subparsers.add_parser(
        "lumacore", help="Install or manage LumaCore (Steam ownership emulation)"
    )
    lc_parser.add_argument(
        "--install", action="store_true", help="Download and install LumaCore (default)"
    )
    lc_parser.add_argument(
        "--uninstall",
        action="store_true",
        help="Deactivate LumaCore and remove its DLLs",
    )
    lc_parser.add_argument(
        "--status", action="store_true", help="Check LumaCore installation status"
    )
    lc_parser.add_argument(
        "--check-update",
        action="store_true",
    ...
    lc_parser.add_argument(
        "--force", action="store_true", help="Bypass the update-check cache"
    )
    steam_options(lc_parser)
    lc_parser.set_defaults(handler=cmd_lumacore)

    stub_parser = subparsers.add_parser(
        "steamstub", help="Remove SteamStub DRM via Steamless"
    )
    stub_parser.add_argument(
        "game_path", help="Game folder (or single file with --file)"
    )
    stub_parser.add_argument(
        "--file", action="store_true", help="Treat game_path as a single executable"
    )
    stub_parser.add_argument(
        "--restore",
        action="store_true",
        help="Restore all .steamstub.bak backups in the folder",
    )
    stub_parser.add_argument(
        "--dry-run",
        action="store_true",
        help="List candidate executables without unpacking",
    )
    stub_parser.add_argument(
        "--yes", "-y", action="store_true", help="Skip confirmation prompts"
    )
    ...
    crack_parser = subparsers.add_parser(
        "crack", help="Search CrakFiles and apply a community fix to a game folder"
    )
    crack_parser.add_argument("game_path", help="Target game folder")
    crack_parser.add_argument(
        "--game-name", help="Game name to search for in the CrakFiles list"
    )
    crack_parser.add_argument(
        "--list",
        action="store_true",
        help="Only list matching games; no files are changed",
    )
    crack_parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Preview without downloading or applying anything",
    )
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    try:
        ...
        if getattr(args, "debug", False):
            ...
            print(
                f"unexpected error: {exc} — re-run with --debug for a full traceback",
                file=sys.stderr,
            )
        return 1


def _require_compat() -> None:
    if not HAS_COMPAT:
        detail = f" ({COMPAT_IMPORT_ERROR})" if COMPAT_IMPORT_ERROR else ""
        raise FourUFourFreeError(
            f"Optional compatibility components are unavailable{detail}"
        )
...
def _confirm(prompt: str, assume_yes: bool = False) -> bool:
    ...
    if not sys.stdin.isatty():
        raise FourUFourFreeError(
            f"{prompt} Re-run with --yes to proceed non-interactively."
        )
    try:
...
def cmd_dlc_unlocker(args, store: ConfigStore) -> int:
        ...
        payload = [
            {
                ...
                "installed": bool(
                    game_path.exists()
                    and game_path.is_dir()
                    and inst.is_installed(game_path)
                ),
            }
    ...
    if not args.unlocker:
        raise FourUFourFreeError(
            "Specify --unlocker (creamapi, smokeapi, uplay-r1, uplay-r2)"
        )
    ...
    if args.uninstall:
        if not _confirm(
            f"Uninstall {args.unlocker} from {game_path}?", getattr(args, "yes", False)
        ):
            print("Cancelled.")
            return 1
        uninstallers = {
        ...
        AuditLog().record(
            ...
            {"game": str(game_path), "unlocker": args.unlocker, "ok": ok},
        )
        if args.json:
            _json({"success": ok})
        print(
            f"{'Uninstalled' if ok else 'Failed to uninstall'} {args.unlocker}: {game_path}"
        )
    ...
    if args.unlocker == "creamapi":
        if not _confirm(
            f"Install {args.unlocker} into {game_path}? Original DLLs are backed up.",
        ...
        if not _confirm(
            ...
            print("Cancelled.")
            return 1
        ok = SmokeAPIUnlocker().install(
            game_path,
            dlc_ids,
            args.app_id,
            smokeapi_dir=_dll_dir_for(downloader, "smokeapi"),
        )
        ...
        if not _confirm(
            ...
            print("Cancelled.")
            return 1
        ok = UplayR1Unlocker().install(
            game_path,
            dlc_ids,
            args.app_id,
            unlocker_dir=_dll_dir_for(downloader, "uplay-r1"),
        )
    else:
        if not _confirm(
            f"Install {args.unlocker} into {game_path}? Original DLLs are backed up.",
            ...
            print("Cancelled.")
            return 1
        ok = UplayR2Unlocker().install(
            game_path,
            dlc_ids,
            args.app_id,
            unlocker_dir=_dll_dir_for(downloader, "uplay-r2"),
        )
    ...
    print(
        f"{'Installed' if ok else 'Failed to install'} {args.unlocker} into {game_path}"
    )
    return 0 if ok else 1
...
def cmd_lumacore(args, store: ConfigStore) -> int:
    ...
    if args.uninstall:
        ...
        AuditLog().record(
            "lumacore.deactivated",
            {"steam_root": str(steam_path), "ok": ok, "message": message},
        )
        if args.json:
...
def cmd_steamstub(args, store: ConfigStore) -> int:
    ...
    if not unpacker.is_available():
        raise FourUFourFreeError(
            "Steamless not found under third_party/. SteamStub removal unavailable."
        )

    if args.restore:
        if not _confirm(
            f"Restore all .steamstub.bak backups in {game_path}?",
            getattr(args, "yes", False),
        ):
            print("Cancelled.")
            return 1
        ...
        AuditLog().record(
            "steamstub.restored", {"path": str(game_path), "restored": restored}
        )
        if args.json:
    ...
    if args.file:
        ...
        if args.dry_run:
            ...
            return 0
        if not _confirm(
            f"Run Steamless on {target}? A .steamstub.bak backup is created.",
            getattr(args, "yes", False),
        ):
            print("Cancelled.")
            return 1
    ...
    if args.dry_run:
        exe_files = [
            f for f in game_path.rglob("*.exe") if not unpacker._should_skip(f)
        ]
        if args.json:
        ...
        return 0
    if not _confirm(
        f"Scan and unpack SteamStub executables under {game_path}?",
        getattr(args, "yes", False),
    ):
        print("Cancelled.")
        return 1
    ...
    AuditLog().record(
        "steamstub.directory_unpacked", {"path": str(game_path), "count": count}
    )
    if args.json:
...
def cmd_crack(args, store: ConfigStore) -> int:
    ...
    if args.list:
        ...
        matches = (
            search_crack_games(args.game_name or "", games) if args.game_name else games
        )
    ...
    if args.dry_run:
        print(
            f"Dry-run: would fetch CrakFiles list and apply the chosen fix to {game_folder}"
        )
        return 0

    ok = apply_crack_fix(args.game_name, game_folder)
    AuditLog().record(
        "crack.applied", {"folder": str(game_folder), "name": args.game_name, "ok": ok}
    )
    if args.json:
        _json({"success": ok})
    return 0 if ok else 1
