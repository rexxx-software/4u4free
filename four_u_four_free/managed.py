...
def quarantine_managed_lua(
    ...
    matches = [info.path for info in list_managed_lua(steam_root) if _matches_app(info, app_id)]
    payload: Dict[str, object] = {"app_id": app_id, "applied": False, "files": [str(path) for path in matches]}
    if not matches or not apply:
        return payload

    stamp = datetime.now().strftime("%Y%m%d-%H%M%S-%f")
    destination = ((quarantine_root or default_data_dir() / "quarantine") / f"{app_id}-{stamp}").resolve(strict=False)
    if destination.exists():
        raise FourUFourFreeError(f"Quarantine destination already exists: {destination}")
    snapshot = create_backup(steam_root, output=backup_output, libraries=list_libraries(steam_root))
    moved: List[Path] = []
    try:
