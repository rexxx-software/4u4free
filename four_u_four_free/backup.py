...
def _source_files(
    steam_root: Path, libraries: Iterable[Path]
...
def create_backup(
    ...
    destination = (output or default_data_dir() / "backups" / stamp).resolve(
        strict=False
    )
    ...
    root_labels = {
        str(path).casefold(): ("steam" if path == root else f"library-{index}")
        for index, path in enumerate(normalized_libraries)
    }
    try:
        destination.mkdir(parents=True, exist_ok=False)
        for source, target_root, target_relative in _source_files(
            root, normalized_libraries
        ):
