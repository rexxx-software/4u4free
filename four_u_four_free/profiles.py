...
def account_id_from_steam_id64(value: str) -> str:
    ...
    if numeric > 0xFFFFFFFFFFFFFFFF:
        raise FourUFourFreeError(f"SteamID64 is outside the unsigned 64-bit range: {value!r}")
...
def list_profiles(steam_root: Path) -> List[SteamProfile]:
    ...
    if users_path.is_file():
        ...
        entries = {key: value for key, value in users.items() if isinstance(value, dict)}
    ...
    if userdata.is_dir():
        try:
            ...
            raise FourUFourFreeError(f"Could not read Steam userdata directory {userdata}: {exc}") from exc
        for path in paths:
            if path.is_dir() and path.name.isdigit() and path.name not in seen_accounts:
                profiles.append(SteamProfile("", path.name, "", "", False, False, "", path))
    return sorted(profiles, key=lambda profile: (not profile.most_recent, profile.persona_name.casefold(), profile.account_id))
