...
def _validated_games(snapshot: Dict[str, object], label: str) -> List[Dict[str, object]]:
    ...
    for index, game in enumerate(snapshot["games"]):
        if not isinstance(game, dict):
            raise FourUFourFreeError(f"Snapshot {label} has a non-object game at index {index}")
        app_id = game.get("app_id")
        if not isinstance(app_id, str) or not app_id.isdigit():
            raise FourUFourFreeError(f"Snapshot {label} has an invalid App ID at index {index}")
        if app_id in seen:
            raise FourUFourFreeError(f"Snapshot {label} contains duplicate App ID {app_id}")
        seen.add(app_id)
        for field in ("name", "install_dir", "build_id", "last_updated", "library", "manifest"):
            if not isinstance(game.get(field), str):
                raise FourUFourFreeError(f"Snapshot {label} has an invalid {field!r} for App ID {app_id}")
        digest = game.get("manifest_sha256")
        if not isinstance(digest, str) or not _SHA256.fullmatch(digest):
            raise FourUFourFreeError(f"Snapshot {label} has an invalid manifest hash for App ID {app_id}")
        result.append(game)
    return result
...
def write_snapshot(snapshot: Dict[str, object], output: Path, force: bool = False) -> Path:
    _validated_games(snapshot, "generated data")
    destination = output.resolve(strict=False)
    if destination.exists() and not force:
        raise FourUFourFreeError(f"Snapshot already exists: {destination}. Use --force to replace it.")
    try:
        ...
        raise FourUFourFreeError(f"Could not write snapshot {destination}: {exc}") from exc
    return destination
...
def compare_snapshots(before: Dict[str, object], after: Dict[str, object]) -> Dict[str, object]:
    ...
    compared_fields = ("name", "install_dir", "build_id", "last_updated", "library", "manifest", "manifest_sha256")
    for key in sorted(old.keys() & new.keys()):
        ...
        if differences:
            changed.append({"app_id": key, "library": new[key].get("library"), "changes": differences})
    return {
