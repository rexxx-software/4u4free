...
class SaveVault:
    ...
    def create_snapshot(
        ...
        *,
        ...
        try:
            ...
            raise FourUFourFreeError(f"Could not scan save folder {source_path}: {exc}") from exc
        if not files:
        ...
        try:
            ...
            raise FourUFourFreeError(f"Could not create Save Vault snapshot: {exc}") from exc
    ...
    def restore_snapshot(
        ...
        target = Path(destination or snapshot.source_path).expanduser().resolve(strict=False)
        ...
        try:
            with zipfile.ZipFile(archive, "r") as bundle:
                for member in bundle.infolist():
                    ...
                    if (
                        ...
                        if member.is_dir():
                            continue
                        raise FourUFourFreeError("The snapshot contains an unsafe path.")
                    extracted = temporary_root / relative
                    extracted.parent.mkdir(parents=True, exist_ok=True)
                    with bundle.open(member, "r") as source_handle, extracted.open("wb") as output:
            ...
            raise FourUFourFreeError(f"Could not restore Save Vault snapshot: {exc}") from exc
        finally:
