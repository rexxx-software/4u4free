...
class Catalog:
    ...
    def import_lua(self, source: Path, archive_dir: Optional[Path] = None) -> ImportResult:
        ...
        with self._connect() as connection:
            connection.execute(
                ...
                """,
                (digest, str(source.resolve(strict=False)), str(destination), info.inferred_app_id, json.dumps(metadata), now),
            )
    ...
    def games(self, query: Optional[str] = None) -> List[Dict[str, object]]:
        with self._connect() as connection:
            if query:
                ...
                rows = connection.execute("SELECT * FROM games ORDER BY name COLLATE NOCASE").fetchall()
    ...
    def lua_files(self, app_id: Optional[str] = None) -> List[Dict[str, object]]:
        with self._connect() as connection:
            if app_id:
                rows = connection.execute(
                    "SELECT * FROM lua_files WHERE inferred_app_id = ? ORDER BY imported_at DESC", (app_id,)
                ).fetchall()
            else:
                rows = connection.execute("SELECT * FROM lua_files ORDER BY imported_at DESC").fetchall()
    ...
    def stats(self) -> Dict[str, object]:
        with self._connect() as connection:
            ...
            lua_files = connection.execute("SELECT COUNT(*) FROM lua_files").fetchone()[0]
            libraries = connection.execute("SELECT COUNT(DISTINCT library) FROM games").fetchone()[0]
        return {"catalog": str(self.path), "games": games, "lua_files": lua_files, "libraries": libraries}
