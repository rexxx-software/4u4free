...
try:
    from sff.core.processes import is_proc_running, launch_steam_unelevated
    ...
    from sff.game.crack_fix import (
        _badge_summary,
    ...
    from sff.game.launch_options import (
...
class MainWindow(QMainWindow):
    ...
    def _require_sff(self) -> None:
        if not HAS_SFF:
            ...
            raise FourUFourFreeError(f"SteaMidra engine unavailable{detail}")
    ...
    def _render_store(self) -> None:
        ...
        self.store_next.setEnabled(
            self._store_offset + PER_PAGE < self._store_total
        )
    ...
    def _update_download(self, data: dict) -> None:
        ...
        if key not in self._download_rows:
            ...
            install.clicked.connect(
                lambda _checked=False, current_id=str(data["app_id"]),
                current_name=str(data["name"]): self._open_official_steam_install(
    ...
    def _open_official_steam_install(self, app_id: str, name: str = "") -> None:
        ...
        if not normalized.isdigit():
            QMessageBox.warning(self, "Install in Steam", "This row has an invalid App ID.")
            return
    ...
    def _game_combo_data(game: SteamGame) -> dict[str, str]:
        return {
            ...
            "name": game.name,
            "folder": str(
                game.library / "steamapps" / "common" / game.install_dir
            ),
        }
    ...
    def _confirm_open_achievement_manager(self) -> None:
        ...
        if game is None:
            QMessageBox.warning(
                self, "Achievements", "Choose an installed game first."
            )
            return
        if (
    ...
    def _cancel_achievement_batch(self) -> None:
        ...
        message = f"Achievement review queue canceled; {remaining} games skipped.{suffix}"
        self.achievement_batch_status.setText(message)
        self.log(message)
        self._set_status(message)
    ...
    def _confirm_open_stats(self) -> None:
        ...
        try:
            ...
            self._task_error(str(exc), modal=True)
            return
        message = f"Advanced stat editor opened for {game['name']} (process {process_id})"
    ...
    def _build_showcase_tab(self) -> QWidget:
        ...
        self.showcase_table.setSelectionBehavior(QTableWidget.SelectionBehavior.SelectRows)
    ...
    def _analyze_showcase(self) -> None:
        ...
        def task():
            profiles = [profile for profile in list_profiles(self._steam_root()) if profile.steam_id64]
            if not profiles:
                raise FourUFourFreeError(
        ...
        def done(result):
            ...
            for row, achievement in enumerate(achievements):
                ...
                self.showcase_table.setItem(row, 2, QTableWidgetItem(achievement.game_name))
                unlocked = (
                    time.strftime(
    ...
    def _detect_vault_source(self, _checked: bool = False, *, quiet: bool = False) -> None:
        game = self._selected_combo_game(self.vault_game_select)
        if game is None:
            if not quiet:
                QMessageBox.warning(self, "Save Vault", "Choose an installed game first.")
            return
        try:
    ...
    def _refresh_vault_snapshots(self) -> None:
        ...
        for row, snapshot in enumerate(self._vault_snapshots):
            ...
            self.vault_table.setItem(row, 2, QTableWidgetItem(format_bytes(snapshot.total_size)))
    ...
    def _confirm_create_vault_snapshot(self) -> None:
        ...
        if game is None or not source:
            QMessageBox.warning(
                self, "Save Vault", "Choose a game and the folder containing its save files."
            )
            return
        if (
            QMessageBox.question(
    ...
    def _confirm_restore_vault_snapshot(self) -> None:
        ...
        if not isinstance(snapshot, VaultSnapshot):
            ...
            return
        destination = self.vault_source.text().strip().strip('"') or snapshot.source_path
        if (
            QMessageBox.question(
                self,
        ...
        def done(result):
            self._refresh_vault_snapshots()
            message = f"Restored {result.restored_files} save files to {result.destination}"
            self.log(message)
            self._set_status(message)
    ...
    def _build_playtime_tab(self) -> QWidget:
        ...
        self.playtime_method.currentIndexChanged.connect(
            self._select_playtime_method
        )
        ...
        self.playtime_start_button.clicked.connect(
            self._confirm_start_playtime_session
        )
    ...
    def _update_playtime_session(self) -> None:
        ...
        verb = "Headless idling" if self._playtime_session_mode == "headless" else "Tracking"
        self.playtime_status.setText(f"{verb} {session.game_name}  ·  App {session.app_id}")
        self.playtime_remaining.setText(
        ...
        message = f"Playtime goal reached for {session.game_name} (App {session.app_id})"
        self.log(message)
        self._set_status(message)
        QMessageBox.information(
            self,
    ...
    def _stop_playtime_tracking(self) -> None:
        ...
        completion = "Headless helper stopped." if was_headless else "The game remains open."
        self.playtime_remaining.setText(
    ...
    def _refresh_online_fix_status(self, _checked: bool = False) -> None:
        ...
        def done(result):
            ...
            if not profile.generic_supported:
                ...
                state = "Remove incompatible generic flag" if enabled else profile.status
    ...
    def _confirm_toggle_online_fix(self) -> None:
        ...
        def task():
            ...
            if was_running:
                ...
                if not restarted:
                    restart_message = f" Steam was not restarted automatically: {restart_message}"
                else:
        ...
        def done(message):
            ...
            QMessageBox.information(self, "Online Fix updated", f"{message}{backup_note}")
    ...
    def _build_settings_page(self) -> QWidget:
        ...
        self.settings_plugins_enabled = QCheckBox("Enable reviewed local Python plugins")
        plugin_note = QLabel(
    ...
    def _save_settings(self) -> None:
        ...
        if library_text and not Path(library_text).is_dir():
            ...
            return
        if self.settings_plugins_enabled.isChecked() and not self.preferences.plugins_enabled:
            if (
                QMessageBox.question(
                    self,
