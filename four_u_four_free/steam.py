...
def _looks_like_steam(path: Path) -> bool:
    return path.is_dir() and ((path / "steamapps").is_dir() or (path / "config").is_dir())
...
def find_steam_root(explicit: Optional[Path] = None, configured: Optional[str] = None) -> Optional[SteamLocation]:
    candidates: List[Tuple[Path, str]] = []
    if explicit:
        normalized = explicit.expanduser().resolve(strict=False)
        return SteamLocation(normalized, "command-line") if _looks_like_steam(normalized) else None
    if configured:
    ...
    if os.name == "nt":
        for variable in ("PROGRAMFILES(X86)", "PROGRAMFILES"):
            if os.environ.get(variable):
                candidates.append((Path(os.environ[variable]) / "Steam", f"default:{variable}"))
    else:
        candidates.extend(
            [
...
def require_steam_root(explicit: Optional[Path] = None, configured: Optional[str] = None) -> SteamLocation:
    result = find_steam_root(explicit, configured)
    if result is None:
        raise FourUFourFreeError("Steam was not found. Pass --steam-root or save one with '4u4free config set'.")
    return result
...
def list_games(libraries: Iterable[Path]) -> List[SteamGame]:
    ...
    for library in libraries:
        ...
        for manifest in sorted(steamapps.glob("appmanifest_*.acf")):
            ...
            app_id = get_string(state, "appid") or manifest.stem.removeprefix("appmanifest_")
            games.append(
                SteamGame(
                    app_id=app_id,
...
def doctor(explicit: Optional[Path] = None, configured: Optional[str] = None) -> Dict[str, object]:
