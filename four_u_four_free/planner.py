...
def make_import_plan(lua: LuaInfo, steam_root: Path) -> ImportPlan:
    ...
    actions = [
        PlanAction(1, "validate", str(lua.path), "Validate and parse the source Lua without executing it.", True),
        PlanAction(2, "backup", str(steam_root), "Create a checksummed snapshot of relevant Steam state.", True),
        PlanAction(3, "copy", str(target), "Copy the Lua into Steam's plug-in directory.", False),
        PlanAction(4, "config", str(steam_root / "config" / "config.vdf"), "Apply depot keys using a structured VDF update.", False),
        PlanAction(5, "manifest", str(steam_root / "steamapps"), "Resolve and install authorized manifests, then update ACF metadata.", False),
    ]
    warnings = [
    ...
    if lua.tokens:
        warnings.append("The file contains access tokens; command output redacts them by default.")
    return ImportPlan(
