...
def export_config(store: ConfigStore, output: Path, force: bool = False) -> Path:
    ...
    if destination.exists() and not force:
        raise FourUFourFreeError(f"Export already exists: {destination}. Use --force to replace it.")
    payload = {"format": "4u4free-config", "export_version": 1, "config": asdict(store.load())}
    try:
        ...
        raise FourUFourFreeError(f"Could not export settings to {destination}: {exc}") from exc
    return destination
...
def read_config_export(path: Path) -> AppConfig:
    try:
        ...
        raise FourUFourFreeError(f"Could not read settings export {path}: {exc}") from exc
    if not isinstance(payload, dict) or payload.get("format") != "4u4free-config" or payload.get("export_version") != 1:
    ...
    if unknown:
        raise FourUFourFreeError(f"Settings export contains unknown keys: {', '.join(sorted(unknown))}")
    if config.get("schema_version", 1) not in {1, 2, 3}:
        raise FourUFourFreeError(f"Unsupported configuration schema version: {config.get('schema_version')!r}")
    ...
    def text_mapping(name: str) -> dict[str, str]:
        ...
        for key, item in value.items():
            if not isinstance(key, str) or not key.strip() or not isinstance(item, str) or not item.strip():
                raise FourUFourFreeError(f"Settings export has an invalid {name!r} entry")
    ...
    return AppConfig(
        ...
        download_source=choice(
            "download_source", {"auto", "oureveryday", "hubcap", "ryuu", "depotbox"}, "auto"
        ),
        hide_adult_content=boolean("hide_adult_content", True),
        confirm_downloads=boolean("confirm_downloads", True),
        store_density=choice("store_density", {"compact", "comfortable"}, "comfortable"),
...
def import_config(store: ConfigStore, source: Path, apply: bool = False) -> Dict[str, object]:
    config = read_config_export(source)
    result: Dict[str, object] = {"applied": False, "source": str(source.resolve(strict=False)), "config": asdict(config)}
    if not apply:
        return result
    backup = None
    if store.path.is_file():
        ...
        try:
            ...
            raise FourUFourFreeError(f"Could not back up current config: {exc}") from exc
    store.save(config)
    result.update({"applied": True, "backup": str(backup) if backup else None, "destination": str(store.path)})
    return result
