...
class TokenDirective:
    ...
    def to_dict(self, show_secrets: bool = False) -> Dict[str, str]:
        return {
            "app_id": self.app_id,
            "token": self.token if show_secrets else redact(self.token),
        }
...
class LuaInfo:
    ...
    def to_dict(self, show_secrets: bool = False) -> Dict[str, object]:
        return {
            ...
            "app_directives": [
                item.to_dict(show_secrets) for item in self.app_directives
            ],
...
def inspect_lua(path: Path) -> LuaInfo:
    ...
    if size > MAX_LUA_BYTES:
        raise FourUFourFreeError(
            f"Lua file is larger than the {MAX_LUA_BYTES // (1024 * 1024)} MiB safety limit"
        )
    try:
    ...
    manifests = {
        match.group("depot"): match.group("gid")
        for match in _SET_MANIFEST.finditer(active)
    }
    tokens = [
        TokenDirective(match.group("appid"), match.group("token"))
        for match in _ADD_TOKEN.finditer(active)
    ]
    inferred = (
        path.stem if path.stem.isdigit() else (tokens[0].app_id if tokens else None)
    )
    return LuaInfo(
        path.resolve(strict=False), inferred, app_directives, manifests, tokens
    )
