...
Reads, but never executes, a Lua metadata file. Supported directives include:
...
Prints the ordered actions needed by the analyzed Lua metadata flow and labels each action `ready` or `future`. It does not create files or alter Steam.
